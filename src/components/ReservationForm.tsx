...
const ReservationForm = () => {
  ...
  useEffect(() => {
    ...
    const handleAddToCart = (e: any) => {
      ...
      const qtyToAdd = parseInt(quantity) || 1; // Zde přijímáme zadané množství
        ...
        <>
          ...
          <div className="absolute right-0 mt-4 w-80 md:w-96 bg-white rounded-[2rem] shadow-2xl p-6 md:p-8 border border-slate-100 text-[#0a192f] z-[100]">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setIsOpen(false)} className="p-2 hover:bg-slate-100 rounded-full">✕</button>
            </div>
            ...
            {cart.length === 0 ? (
              ...
              <form name="objednavka" method="POST" onSubmit={handleSubmit} data-netlify="true">
                ...
                <div className="max-h-48 overflow-y-auto mb-6 space-y-3 pr-2 scrollbar-thin">
                  {cart.map((item, i) => (
                    <div key={i} className="flex gap-3 items-center bg-slate-50 p-2 rounded-xl border border-slate-100">
                      ...
                      <button type="button" onClick={() => removeItem(item.name)} className="p-2 text-red-400 hover:text-red-600">✕</button>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3 pt-4 border-t border-slate-100 text-left">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <select name="Prodejna" value={selectedStore} onChange={(e) => setSelectedStore(e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl text-sm outline-none">
                      <option>Petřvald</option><option>Píšť</option><option>Karviná</option><option>Ostrava (Zábřeh)</option>
                    </select>
                    <input type="date" name="Datum" min={minDate} required className="w-full p-3 bg-slate-50 rounded-xl text-sm outline-none" />
                  </div>
                  <textarea name="Poznamka" placeholder="Poznámka k objednávce..." className="w-full p-3 bg-slate-50 rounded-xl text-sm outline-none h-20 resize-none"></textarea>
                  <button type="submit" className="w-full bg-[#0a192f] text-white py-4 rounded-2xl font-bold hover:bg-[#d4af37] transition-all uppercase tracking-widest text-xs">
                    Odeslat rezervaci
                  </button>
                </div>
              </form>
